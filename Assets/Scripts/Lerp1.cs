...
public class Lerp1 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i =0; i < numSphere; i++){
            ...
            float r = 10f;
            startPosition[i] = new Vector3(r * i * Random.Range(-1f, 1f), r * i * Random.Range(-1f, 1f), r * Random.Range(-1f, 1f));        
            ...
            endPosition[i] = new Vector3(
                                    ...
                                    r * (-1 * cos * cos * cos - cos * cos + 2f * cos),
                                    10f);
        }
    ...
    void Update()
    {
        ...
        for (int i =0; i < numSphere; i++){
            ...
            spheres[i].transform.position = Vector3.Lerp(startPosition[i] / time, endPosition[i] / time, lerpFraction);
            ...
            {
                color = Color.HSVToRGB(Mathf.Abs(hue * Mathf.Sin(time))/time, Mathf.Cos(time), 2f + Mathf.Cos(time)/time); // Full saturation and brightness
            }
            else
            {
                color = Color.HSVToRGB(0f, 0f, 0f); // full negative saturation and brightness
            }
            sphereRenderer.material.color = color;
        }
    }
